...
本章的目的是教您如何制作有用的图形。尽快使用 ggplot2。您将学习以下方面的基础知识 ggplot（）和一些有用的“食谱”，使最重要情节。 ggplot（）可以让您仅用几行就可以绘制出复杂的图代码，因为它基于丰富的基础理论，即图形。在这里，我们将跳过理论，而将重点放在实践上，而在后面的章节中，您将学习如何使用的完整表达能力语法。

在本章中，您将学习：

- 关于 ggplot2 随附的 `mpg` 数据集，第 15 节 \ @ref（燃料经济数据）。

- 每个情节的三个关键组成部分：数据，美学和几何图形 \ @ref（基本用途）。

- 如何通过美学将更多变量添加到情节中\ @ref（美学）。

- 如何使用由构面创建的小倍数，请参见\ @ref（qplot-faceting）。

- 您可以使用多种不同的几何图形来创建不同的几何图形绘图类型，请参见\ @ref（plot-geoms）。

- 如何修改轴，第\ @ref（axes）节。

- 除了显示对象外，您还可以使用绘图对象执行的操作，例如将其保存到磁盘的\ @ref（output）部分。
...
Protection Agency, <http://fueleconomy.gov>. You can access the data by
loading ggplot2: \index{Data!mpg@\texttt{mpg}}
...
- `drv` is the drivetrain: front wheel (f), rear wheel (r) or four
    wheel (4).
...
3.  Apart from the US, most countries use fuel consumption (fuel
    consumed over fixed distance) rather than fuel economy (distance
    travelled with fixed amount of fuel). How could you convert `cty` 
    and `hwy` into the European standard of l/100km?
...
first two arguments to `aes()` are `x` and `y` . Note that I've put each
command on a new line. I recommend doing this in your own code, so it's
easy to scan a plot specification and see exactly what's there. In this
chapter, I'll sometimes use just one line per plot, because it makes it
...
3.  Describe the data, aesthetic mappings and layers used for each of
    ...
    haven't seen all the datasets and functions yet, but use your common
    sense! See if you can predict what the plot will look like before
    running the code.
...
learn how to override them in Chapter \@ref(scale-colour).
...
Section \@ref(setting-mapping). See `vignette("ggplot2-specs")` for the
...
learn more about faceting in Section \@ref(facet), but it's such a

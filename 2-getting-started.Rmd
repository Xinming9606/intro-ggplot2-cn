...
ggplot2将数据（例如，`f1`，`r`，`4`）转换为带有**标度**的美学对象（例如，`red`，`yellow`，`green`）的绘图。绘图中的每个绘图贴图都有一个标度。 标度还可以创建参考线，轴或图例，使您可以读取图，将美学属性值转换回数据值。现在，我们将继续使用ggplot2提供的默认标度。您将在第\ @ref（scale-colour）章中学习如何覆盖它们。

要了解有关上一散点图中的离群变量，我们可以将class变量映射到colour：
...
这为每个点提供了与其类别相对应的唯一颜色。图例使我们能够从颜色中读取数据值，向我们展示了燃油经济性极高的是两座汽车(two seaters)，引擎大但车身轻巧。

如果您想将美学元素设置为固定值而不使用标度，请在`aes（）`的外层执行。比较以下两行代码的区别
...
在第一个图中，值“ blue”被映射为为粉红色，并且有图例。 在第二个图中，点被设置为R蓝色。这是一项重要的技术，您将在小节 \@ref(setting-mapping). 请参阅`vignette（“ ggplot2-specs”）`如何设置颜色和其他美学元素。

不同类型的美学属性与不同类型的变量配合使用效果更好。例如，颜色和形状可以很好地与分类变量配合使用，而尺寸对于连续变量则可以配合使用。大数据量也会有所不同：如果有大量数据，则很难区分不同的组。另一种解决方案是使用刻面，如下所述。

当在图中使用美学元素时，less is more。很难同时看到颜色，形状和大小之间的关系，因此在使用美学元素时要适量。与其尝试制作一个可以一次显示所有内容的非常复杂的图，不如看看是否可以创建一系列讲述故事的简单图，带领读者从无知到了解。
